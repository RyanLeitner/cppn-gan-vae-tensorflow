...
class batch_norm(object):
    ...
    def __init__(self, batch_size, epsilon=1e-5, momentum = 0.1, name="batch_norm"):
        with tf.compat.v1.placeholder(dtype=tf.float32, name=name) as scope:
    ...
    def __call__(self, x, train=True):
        ...
        with tf.compat.v1.placeholder(dtype=tf.float32, name=self.name) as scope:
...
def conv2d(input_, output_dim,
           ...
           name="conv2d"):
    with tf.compat.v1.placeholder(name):
...
def deconv2d(input_, output_shape,
    ...
    with tf.compat.v1.placeholder(name):
        ...
        w = tf.compat.v1.get_variable('w', [k_h, k_h, output_shape[-1], input_.get_shape()[-1]],
...
def linear(input_, output_size, scope=None, stddev=0.02, bias_start=0.0, with_w=False):
    ...
    with tf.compat.v1.placeholder(scope or "Linear"):
...
def fully_connected(input_, output_size, scope=None, stddev=0.1, with_bias = True):
    ...
    with tf.compat.v1.placeholder(scope or "FC"):
        matrix = tf.compat.v1.get_variable("Matrix", [shape[1], output_size], tf.float32,
            tf.random_normal_initializer(stddev=stddev))
        ...
        if with_bias:
            bias = tf.compat.v1.get_variable("bias", [1, output_size],
                initializer=tf.random_normal_initializer(stddev=stddev))
